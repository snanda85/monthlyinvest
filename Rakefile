...
posts_dir       = "posts/_posts"    # directory for blog files
...
multitask :push do
  ...
  Rake::Task[:copydot].invoke(build_dir, deploy_dir)
...
desc "Set up deploy folder and deploy branch for Github Pages deployment"
task :init, :repo do |t, args|
  ...
  if branch == 'master'
    ...
    unless !build_dir.match("#{project}").nil?
  ...
  cd "#{deploy_dir}" do
    ...
    rakefile.sub!(/deploy_method(\s*)=(\s*)(["'])[\w-]*["']/, "deploy_method\\1=\\2\\3push\\3")
